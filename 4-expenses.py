...
def add_expense(expenses: List[Union[int, str]], value: int) -> List[Union[int, str]]:
    ...
    # get_total(expenses) # Печатает все расходы с их индексами
    return expenses


def get_total(expenses: List[int]):  # Показывает сумму расходов
...
def get_total_sum(expenses: List[int]):
...
def delete_expence(expenses: List[int], index: int):
    ...
    if 0 < user_select_index < len(expenses):
        ...
        print(
            f"Расход удален. Сумма оставшихся расходов: {get_total_sum(expenses)}")
    else:
...
def get_average(expenses: List[int]):  # Показывает средний расход
    ...
    total = get_total_sum(expenses)
    if len(expenses) > 1:
        print(f"Средний расход: {total / (len(expenses) - 1)}")
    return 0
...
def print_report(expenses: List[int]):  # Печатает красивый отчет
    ...
    get_total(expenses)
    print(f"Сумма: {get_total_sum(expenses)}")
...
while user_select not in menus:
    ...
    match user_select:
        ...
        case "2":
            get_total(expenses)  # показывает все расходы
        case "3":
            # get_total(expenses)  # возвращает сумму
            print(f"Сумма всех расходов: {get_total_sum(expenses)}")
        case "4":
